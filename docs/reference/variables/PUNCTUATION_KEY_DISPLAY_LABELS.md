---
id: PUNCTUATION_KEY_DISPLAY_LABELS
title: PUNCTUATION_KEY_DISPLAY_LABELS
---

```ts
const PUNCTUATION_KEY_DISPLAY_LABELS: object;
```

Defined in: [display-labels.ts:85](https://github.com/TanStack/hotkeys/blob/main/packages/hotkeys/src/display-labels.ts#L85)

## Type Declaration

### -

```ts
readonly -: "Minus" = 'Minus';
```

### ,

```ts
readonly ,: "Comma" = 'Comma';
```

### ;

```ts
readonly ;: "Semicolon" = 'Semicolon';
```

#### .

```ts
readonly .: "Period" = 'Period';
```

### \[

```ts
readonly [: "Left Bracket" = 'Left Bracket';
```

### \]

```ts
readonly ]: "Right Bracket" = 'Right Bracket';
```

### \

```ts
readonly \: "Backslash" = 'Backslash';
```

### \`

```ts
readonly `: "Backquote" = 'Backquote';
```

### =

```ts
readonly =: "Equal" = 'Equal';
```
